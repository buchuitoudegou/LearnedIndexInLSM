...
data=random_readonly_uniform_small
...
for compaction_policy in 0 1 2; do
for bpk in 1 5 10 50 100 500; do
# fence pointer
MOD=0
for block in 1024 4096 16384 65536 262144 1048576 4194304 16777216 67108864; do
./build/read_cold -f ${data_file} -r ${query_file} --inputlen 64000001 --compactionpolicy ${compaction_policy} -b ${bpk} -e 10 -k ${keysize} -v ${valuesize} -d ${db_loc} --SST ${SST} --blocksize ${block} --buffersize ${buffersize} -m 0 --modelmode ${modelmode} --dataset ${data} --exp 0 -w > ./log/64Mlongitude.log
./build/read -f ${data_file} -r ${query_file} --compactionpolicy ${compaction_policy} -b ${bpk} -e 10 -k ${keysize} -v ${valuesize} -d ${db_loc} --SST ${SST} --blocksize ${block} --buffersize ${buffersize} -m 0 --modelmode ${modelmode} --dataset ${data} --exp 0 -w > ./log/64Mlongitude.log
done
MOD=6
./build/read_cold -f ${data_file} -r ${query_file} --inputlen 64000001 --compactionpolicy ${compaction_policy} -b ${bpk} -e 10 -k ${keysize} -v ${valuesize} -d ${db_loc} --SST ${SST} --blocksize ${blocksize} --buffersize ${buffersize} -m $MOD --modelmode ${modelmode} --dataset ${data} --exp 0 -w > ./log/64Mlongitude.log
if true; then
# PLR
