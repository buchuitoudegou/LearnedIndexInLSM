...
void DBImpl::CompactMemTable(MemTable *table) {
    mutex_.AssertHeld();
    // std::cout<<"Compacting memtables with para"<<std::endl;
...
Status DBImpl::Get(const ReadOptions& options, const Slice& key,
  ...
  // Unlock while reading from files and memtables
  {
    ...
    if (mem->Get(lkey, value, &s)) {
      ...
      adgMod::MemtableDuration += duration_microsecond;
#endif
    ...
    if (adgMod::MOD >= 7 && s.ok()) {
      ...
      *value = std::move(vlog->ReadRecord(value_address, value_size));
      ...
      auto afterTime = std::chrono::steady_clock::now();
      double duration_microsecond = std::chrono::duration<double, std::micro>(afterTime - beforeTime).count();
      adgMod::GetvalueDuration += duration_microsecond;
#endif
    }
    mutex_.Lock();
  }
...
Status DB::Open(const Options& options, const std::string& dbname, DB** dbptr) {
  ...
  *dbptr = nullptr;
  ...
  if(adgMod::LoadModelTimer){
    adgMod::LoadModelDuration += duration_microsecond;
  }
  ...
  auto dbopenendTime = std::chrono::steady_clock::now();
  double dbopenend_microsecond = std::chrono::duration<double, std::micro>(dbopenendTime - dbopenbeforeTime).count();
  if(adgMod::LoadModelTimer){
    adgMod::DBOpenDUration+= dbopenend_microsecond;
  }
  return s;
}
