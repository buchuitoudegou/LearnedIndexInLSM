...
// Callback from TableCache::Get()
    ...
    Status Version::Get(const ReadOptions &options, const LookupKey &k,
        ...
        for (int level = 0; level < config::kNumLevels; level++) {
            ...
            if (level == 0) {
                ...
                if (adgMod::MOD == 6 || adgMod::MOD == 7 || adgMod::MOD == 9) {
                    ...
                    //                        size_t index;
                        ...
                        // std::cout<<"Not using model"<<std::endl;
            ...
            adgMod::FindFileDuration += duration_microsecond;
            instance->PauseTimer(0);
            // printf("Files listed!\n");
            // std::cout<<"Key: "<<adgMod::SliceToInteger(user_key)<<" candidate number: "<<num_files<<std::endl;
            for (uint32_t i = 0; i < num_files; ++i) {
                ...
                // std::cout<<"In candidate file: "<<f->number<<std::endl;
                ...
                if (adgMod::MOD == 0 || adgMod::MOD == 8) {
                    ...
                    // std::cout<<"usinig level model?"<<learned<<std::endl;
                    s = vset_->table_cache_->Get(options, f->number, f->file_size, ikey,
                ...
                adgMod::FileModelDuration += duration_microsecond;
                auto temp = instance->PauseTimer(6, true);

                // std::cout<<"staus in candiate loop: "<<s.ToString()<<std::endl;
    ...
    void VersionSet::ApplyToModel(VersionEdit *edit, Version *previous, Version *current) {
        ...
        for (int i = 1; i < config::kNumLevels; ++i) {
            ...
            if (changed_level.count(i) == 0) {
                // std::cout<<"\tdoing operator="<<std::endl;
...
//   in/out compaction_files: List of files to extend by adding boundary files.
    ...
    bool Version::FillLevel(const ReadOptions &options, int level) {
        ...
        for (int j = 0; j < files_[level].size(); ++j) {
            ...
            data->string_keys.emplace_back(std::make_pair(std::string(file->smallest.user_key().data(), file->smallest.user_key().size()), 0));
            data->string_keys.emplace_back(std::make_pair(std::string(file->largest.user_key().data(), file->largest.user_key().size()), 0));
    ...
    void Version::FileLearn() {
        for (int i = 0; i < config::kNumLevels; ++i) {
            // std::cout<<"level "<< i <<" file number: "<< files_[i].size() <<std::endl;
            for (FileMetaData *file_meta : files_[i]) {
