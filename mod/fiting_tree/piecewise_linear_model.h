...
public:
    ...
    Ft_Segment<X, Y> get_segment()
    {
        if (points_in_segment == 1)
            return Ft_Segment<X, Y>((X)first_point.x, (Y)first_point.y, (X)last_point.x, 1);
        ...
        return Ft_Segment<X, Y>(X(first_point.x), Y(first_point.y), X(last_point.x), slope);
    }
...
auto get_all_segments(RandomIterator first, RandomIterator last, size_t error)
{
    ...
    std::vector<Ft_Segment<key_type, size_t>> out;
