...
class Segment {
    ...
    bool search(int start, int end, ftKeyType key, ftValueType *v) {
        bool found = false;
        int l = start;
        int r = end;
        while (l <= r) {
...
#define FMaxLeafNodeItemCount (MaxNodeSize/FLeafNodeItermSize) 
    ...
    #define MaxItemInnerNodeLeafValue (BlockSize/(sizeof(ftKeyType) + sizeof(void *)))
    struct traits_inner : stx::btree_default_map_traits<ftKeyType, LeafNodeValueOnDisk>
    {
    ...
    typedef stx::btree_multimap<ftKeyType, LeafNodeValueOnDisk, std::less<ftKeyType>, traits_inner> stx_btree;
...
class FITingTree {
    public:
        ...
        void load_metanode() {
            // std::cout<<"loading meta node"<<std::endl;
    ...
    // FITingTree() = default;
    FITingTree() {
        char* file_name = (char*)"default.txt";
        sm = new StorageManager(true, file_name);
        load_metanode();
    }
    ...
    class Seg {
        public:
            ftKeyType key;             ///< The first key that the segment indexes.
            float slope;    ///< The slope of the segment.
            int32_t intercept; ///< The intercept of the segment.
            int32_t start;
            int32_t end;
        
        Seg(ftKeyType key, float slope, int32_t intercept) : key(key), slope(slope), intercept(intercept) {};

        explicit Seg(size_t n) : key(std::numeric_limits<ftKeyType>::max()), slope(), intercept(n) {};

        explicit Seg(const typename OptimalPiecewiseLinearModel<ftKeyType, size_t>::CanonicalSegment &cs)
            : key(cs.get_first_x()) {
    ...
    void _ts(RandomIt first, RandomIt last, size_t epsilon, std::vector<Seg> &segments) {
        ...
        auto ignore_last = *std::prev(last) == std::numeric_limits<ftKeyType>::max(); // max() is the sentinel value
            ...
            auto in_fun = [&](auto i) {
                ...
                return std::pair<ftKeyType, size_t>(x + flag, i);
            };
    ...
    std::vector<LeafNodeItermOnDisk> write_segment_pgm(Iterm *data, long item_count, std::vector<Seg> segments, bool is_first, int last_block, int next_block) {
        ...
        if (is_first) {
            Iterm _data; 
            _data.key = (ftKeyType)(0);
            _data.value = (ftValueType)(metanode.block_count + 1);
...
//                total_blocks += wroten;
            ...
            _data[0].key = (ftKeyType)(last_block);
            _data[0].value = i == (segments.size() - 1)? (ftValueType)(next_block) : (ftValueType)(metanode.block_count + wroten);
            _data[1].key = (ftKeyType)(copy_size);
...
//                total_offset += (MaxNodeSize - offset);
            ...
            if (!is_first && i == 0 && last_block != 0) {
                ...
                _data[0].value = (ftValueType)(metanode.block_count);
            ...
            if (!is_first && i == (segments.size() - 1) && next_block != 0) {
                ...
                Iterm *_data = (Iterm *)_temp;
                _data[0].key = (ftKeyType)(metanode.block_count);
...
//        std::cout << total_write_size << "," << total_blocks << "," << total_offset << std::endl;
    ...
    std::vector<FLeafNodeIterm> write_segment(Iterm *data, long item_count) {
        ...
        double low_slope = 0;
        ftKeyType original_key = data[0].key;
        double original_location = 0;
        // bool is_first = true;
        long i = 1;
        for (; i < item_count; i++) {
            ftKeyType temp_key = data[i].key;
            ...
            if (temp_slope <= high_slope && temp_slope >= low_slope) {
                ...
                memcpy(_data+1, data + int(original_location), copy_size * ItermSize);
                    ...
                    _data[0].key = (ftKeyType) (metanode.block_count + wroten);
                    _data[0].value = (ftValueType) (copy_size);
                    sm->write_with_size(metanode.block_count, _data, write_size);
                    delete[] _data;
                    if (offset > 0) {
                        char empty[MaxNodeSize - offset];
...
//        std::cout << sm->get_file_size() << std::endl;
        if (hybrid_mode == ALL_DISK)
            ...
            std::vector<std::pair<ftKeyType, LeafNodeValueOnDisk> > pairs(leaf_iterms.size());
    ...
    void _build_tree(Iterm *data, long item_count, std::vector<LeafNodeItermOnDisk> leaf_iterms) {
        ...
        ftKeyType _keys[leaf_node_count];
    ...
    bool _lookup(ftKeyType key, void *address, ftValueType *v) {
    ...
    #if Disk
    bool _get_from_segment_buffer(ftKeyType key, int block_id, ftValueType *v, int *block_count) {
    ...
    void scan_in_leaf_node(ftKeyType key, int start_block, int pos, ftValueType *v, int *block_count, int item_count, int item_buffer) {
...
//                *v =_temp[_offset].value;
    ...
    bool search_in_leaf_node(ftKeyType key, int start_block, int pos, ftValueType *v, int *block_count, int item_count, int item_buffer) {
        ...
        while (start <= end) {
            ...
            read_block_or_not(&last_block, start_block + _block, _data2, block_count);
            Iterm * _temp = (Iterm * )_data2;
            // std::cout<<"target key:"<<key<<" got key:"<<_temp[_offset].key<<std::endl;
            if (_temp[_offset].key == key) {
                found = true;
                *v =_temp[_offset].value;
                break;
    ...
    bool _lookup_disk(ftKeyType key, int block_id, ftValueType *v, int *block_count) {
    ...
    bool lookup_disk(ftKeyType key, int *block_count, ftValueType *v) {
        bool found = false;
        found = _lookup_disk(key, metanode.root_block_id, v, block_count);
        return found;
    }

    bool lookup_leaf_disk(ftKeyType key, int *block_count, ftValueType *v) {
...
//            std::cout << "should not happen" << std::endl;
    ...
    size_t get_index_size() {
        return metanode.block_count * BlockSize;
    }
    int lookup(ftKeyType key, int *block_count, ftValueType *v) {
    ...
    std::vector<LeafNodeItermOnDisk> add_left_buffer(bool *no_op, ftKeyType key, ftValueType value, int next_block_id) {
        char *_data2 = new char[MaxNodeSize];
        ...
        if (metanode.added_in_buffer == ItermCountOneBlock - 2) { // the first one is meta for next and last block
            // resegment
            #if PGMSegment
            ftKeyType *_xdata20 = new ftKeyType[ItermCountOneBlock-1];
            for (int _xdatai = 1; _xdatai < ItermCountOneBlock; _xdatai++) {
                _xdata20[_xdatai-1] = _temp[_xdatai].key;
            }
            std::vector<ftKeyType> _xdata21(_xdata20, _xdata20+(ItermCountOneBlock-1));
            std::vector<Seg> segments; 
    ...
    std::vector<LeafNodeItermOnDisk> add_segment_buffer_for_leaf_disk(bool *no_op, ftKeyType true_key, ftValueType value, stx_btree::iterator it) {
//        LeafNodeItermOnDisk *lnids = (LeafNodeItermOnDisk *)(data + LeaftNodeHeaderSizeOnDisk);
        char *_data2 = new char[MaxNodeSize];
...
#if Disk
#if PGMSegment
            ftKeyType *_xdata20 = new ftKeyType[buffer_size];
            for (int _xdatai = 0; _xdatai < buffer_size; _xdatai++) {
                _xdata20[_xdatai] = buffer[_xdatai].key;
            }
            std::vector<ftKeyType> _xdata21(_xdata20, _xdata20+buffer_size);
            std::vector<Seg> segments;
            _ts(_xdata21.begin(), _xdata21.end(), error_bound, segments);
    ...
    std::vector<LeafNodeItermOnDisk> add_segment_buffer(bool *no_op, int start_block, ftKeyType true_key, ftValueType value, char *data, int i, int current_block_id) {
            LeafNodeItermOnDisk *lnids = (LeafNodeItermOnDisk *)(data + LeaftNodeHeaderSizeOnDisk);
            char *_data2 = new char[MaxNodeSize];
    ...
    FBuildStatus insert_key(ftKeyType key, ftValueType value, int *block_count, int current_block_id, ftKeyType true_key) {
        // load the leaf node
        FBuildStatus bs;
        char *data = new char[MaxNodeSize];
    ...
    void insert_disk(ftKeyType key, ftValueType value) {
        //int *block_count, int current_block_id
        int block_count = 0;
        ftKeyType search_key = key;
    ...
    void insert_key_leaf_disk(ftKeyType key, ftValueType value) {
        // search in inner nodes
        stx_btree::iterator it = inner_btree.find_for_disk(key);
        // find the right leaf segment to search
...
//            metanode.block_id;
    ...
    void scan_leaf_disk(ftKeyType key, ftKeyType *v, int *block_count, int len) {
        // search in inner nodes
        stx_btree::iterator it = inner_btree.find_for_disk(key);
        // find the right leaf segment to search
    ...
    void scan_in_a_segment(int start, int end, int key, int start_block, int *block_count,
                           int iterm_count, int len, ftValueType *v, bool is_left_most) {
//        int start = std::max<int>(pos - error_bound, 0) + 2;
//        int end = std::min<int>(pos + error_bound+2, iterm_count) + 2;
    ...
    void simple_scan_disk(ftKeyType key, int block_id, ftKeyType *v, int *block_count, int len) {
        if (key < metanode.original_key) { // must in the first segment buffer if have
            return scan_in_a_segment(1, metanode.added_in_buffer, key, metanode.block_id, block_count,
                              metanode.added_in_buffer, len, v, true);
        }
