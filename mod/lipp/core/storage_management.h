#define lippKeyType long long int
#define lippValueType int
...
class StorageManager {
    ...
    typedef struct {
        ...
        // 2 child
        // 3 data
        int tag; // we know the type from the bitmap in header
        union {
            struct {
                T key;
                P value;
    ...
    public:
        ...
        void readall(){
            ...
            //     read_block(data,i);
            //     std::vector<char> tmp(data, data + BLOCK_SIZE);
            //     // std::cout<<"tmp.size is:"<<tmp.size()<<std::endl;
            //     ALLDATA.push_back(tmp);
            // }
            int totalSize = (TOTAL_BLOCK+2) * BLOCK_SIZE;
            ...
            for (int i = 0; i < TOTAL_BLOCK+2; i++) {
                std::vector<char> block(buffer.begin() + i * BLOCK_SIZE, buffer.begin() + (i + 1) * BLOCK_SIZE);
                ALLDATA.push_back(block);
            }
        }

        void init(const char *fn, bool is_first) {
            if (is_first) {
                fp = fopen(fn,"wb");
                // char empty_block[BLOCK_SIZE];
                // MetaNode mb;
        ...
        bool searchv2(T key, P *value){
            ...
            int offset2 = 0;
            bool checkflag = true;
            // std::cout<<"In side search:"<<key<<std::endl;
            // std::cout<<"TOTALBLOCK is:"<<TOTAL_BLOCK<<" ALLDATA size:"<<ALLDATA.size()<<std::endl;
            // *bc = 0;
            // load_metablock();
            // *bc += 1;
            while (true) {
                // std::cout<<"In while, block1:"<<block1<<std::endl;
                // read node header
                // *lc += 1;
                // read_block(data, block1);
                if(block1>=ALLDATA.size()) block1 = ALLDATA.size()-1;
                std::vector<char> tmp = ALLDATA[block1];
                // std::cout<<"tmp.size is:"<<tmp.size()<<std::endl;
                ...
                // if(checkflag) std::cout<<"slope:"<<nhd->slope<<" key:"<<key<<" intercept:"<<nhd->intercept<<std::endl;
                ...
                // if(checkflag) std::cout<<"pos:"<<pos<<std::endl;
                // read iterm
                ...
                if (pos < c1) {
                    ...
                    // std::cout<<"pos:"<<pos<<" c1:"<<c1<<" _b:"<<_b<<" offset2:"<<offset2<<std::endl;
                    // read_block(data, block2);
                    // std::cout<<"block2:"<<block2<<" ALLDATA size:"<<ALLDATA.size()<<std::endl;
                    if(block2>=ALLDATA.size()) block2 = ALLDATA.size()-1;
                    std::vector<char> tmp = ALLDATA[block2];
                    // std::cout<<"block2 read done"<<std::endl;
                    std::copy(tmp.begin(), tmp.end(), data);
                    
                    // *bc += 1;
                    itd = (ItemD *) (data + offset2*ItemDSize);
                }
                // if(checkflag) std::cout<<"tag:"<<itd->tag<<std::endl;
                ...
                if (itd->tag == 1) {
                    ...
                    // if(checkflag) std::cout<<"block1:"<<block1<<std::endl;
                    if(block1<0||block1>60000) return false;
                } else {
                    // if(checkflag) std::cout<<"got key:"<<itd->comp.data.key<<" and we want:"<<key<<std::endl;
                    if (itd->comp.data.key == key) {
                        *value = itd->comp.data.value;
                        // if(checkflag)std::cout<<"found!"<<std::endl;
                        return true;
                    }
                    return false;
                }
        ...
        int predict_pos(NodeHeaderD nhd, T key) {
            double v = nhd.slope * static_cast<long double>(key) + nhd.intercept;
            int pos = 0;
            // std::cout<<"slope:"<<nhd.slope<<" key:"<<key<<" intercept:"<<nhd.intercept<<std::endl;
            // std::cout<<"predicting pos: "<<v<<std::endl;
            if (v > std::numeric_limits<int>::max() / 2) {
                pos = nhd.num_items - 1;
            } else if (v < 0) {
                pos = 0;
        ...
        void bulk_load_disk(T *_keys, P *_values, int _size, int *r_block = nullptr, int *r_offset = nullptr) {
            ...
            while (!s.empty()) {
                ...
                if (end - begin == 2) {
                    // std::cout<<"building node two with"<<_keys[begin]<<" and "<<_keys[begin+1]<<std::endl;
                    ...
                    // std::cout<<"Build gap cnt: "<< BUILD_GAP_CNT <<std::endl;
                    ...
                    // std::cout<<"Flag3.2"<<std::endl;
                    {
                        ...
                        if (D * 3 <= size) {
                            ...
                            // std::cout<<"size:"<<size<<"mid1_pos:"<<mid1_pos<<" mid2_pos:"<<mid2_pos<<std::endl;
                            ...
                            // std::cout<<"keys[mid1_pos]:"<<keys[mid1_pos]<<" keys[mid1_pos + 1]:"<<keys[mid1_pos + 1]<<std::endl;
                            // std::cout<<"keys[mid2_pos]:"<<keys[mid2_pos]<<" keys[mid2_pos + 1]:"<<keys[mid2_pos + 1]<<std::endl;
                            ...
                            // std::cout<<"mid1_key:"<<mid1_key<<" mid2_key:"<<mid2_key<<std::endl;
                            node.slope = (mid2_target - mid1_target) / (mid2_key - mid1_key);
                            node.intercept = mid1_target - node.slope * mid1_key;
                        }
                    }
                    ...
                    // std::cout<<"After 3.3 node.intercept"<<node.intercept<<" node.slope"<<node.slope<<std::endl;
                    node.intercept+= lr_remains;
                    node.num_items += lr_remains * 2;
                    // std::cout<<"More After 3.3 node.intercept"<<node.intercept<<" node.slope"<<node.slope<<std::endl;
                    ...
                    for (int item_i = predict_pos(node, keys[0]), offset = 0; offset < size; ) {
                        ...
                        // std::cout<<"Flag3.4.2"<<std::endl;
                        if (next == offset + 1) {
                            // std::cout<<"Flag3.4.2.1"<<std::endl;
                            items[item_i].tag = 3;
                            // std::cout<<"Flag3.4.2.12"<<std::endl;
                            items[item_i].comp.data.key = keys[offset];
                            // std::cout<<"Flag3.4.2.13"<<std::endl;
                            items[item_i].comp.data.value = values[offset];
                        } else {
                            // std::cout<<"Flag3.4.2.2 item_i:"<<item_i<<std::endl;
                            items[item_i].tag = 2;
                            // std::cout<<"Flag3.4.2.22"<<std::endl;
                            NodeHeaderD _nhd;
                            Segment tmp = (Segment){SEGMENTID, begin + offset, begin + next, level + 1, _nhd};
                            // std::cout<<"Flag3.4.2.222"<<std::endl;
                            s.push(tmp);
                            // std::cout<<"Flag3.4.2.23"<<std::endl;
                            _map m;
                            m.parent_id = current_segid;
                            // std::cout<<"Flag3.4.2.24"<<std::endl;
                            m.i = item_i;
                            mappings.push_back(m);
                            SEGMENTID += 1;
                        }
                        // std::cout<<"Flag3.4.3"<<std::endl;
                        if (next >= size) {
                            break;
                        } else {
                            item_i = next_i;
                ...
                add.block = _block;
                add.offset = _offset;
                // if(_block>max_block) max_block = _block;
        ...
        void cal_total_block(){
            ...
            int block1 = mb.root_block;
            int offset1 = mb.root_offset;
            read_block(data1, block1);
            nhd = (NodeHeaderD *) (data1 + offset1);
            _root = *nhd;

            s.push(Segment(0, _root));
            addr.push(NodeAddress(block1, offset1));
            char data[BLOCK_SIZE];
            char data2[BLOCK_SIZE];
            while (!s.empty()) {
                int begin = s.top().first;
                NodeHeaderD node = s.top().second;
                lipp_size += sizeof(node);
                const int SHOULD_END_POS = begin + node.size;
                s.pop();
                int _block = addr.top().first;
                int _offset = addr.top().second;
                ...
                for (int i = 0; i < node.num_items;) {
                    // std::cout<<"Reading block number: "<<_block<<std::endl;
                    read_block(data, _block);
                    if(TOTAL_BLOCK<_block) TOTAL_BLOCK=_block;
                    ItemD *_items = (ItemD *) (data + _offset_);
                    lipp_size += sizeof(*_items);
                    if (c1 > (node.num_items - i)) {
                        c1 = node.num_items - i;
                    }
                    for (int j = 0; j < c1; j++) {
                        if (_items[j].tag == 3) {
                            ...
                            int _b = _items[j].comp.addr.block;
                            int _o = _items[j].comp.addr.offset;
                            // std::cout<<"Reading block number _b: "<<_b<<std::endl;
                            if(TOTAL_BLOCK<_b) TOTAL_BLOCK=_b;
                        }
                    }
        ...
        size_t cal_lipp_size() {
            ...
            int block1 = mb.root_block;
            int offset1 = mb.root_offset;
            // std::cout<<"Reading block number: "<<block1<<std::endl;
            read_block(data1, block1);
            nhd = (NodeHeaderD *) (data1 + offset1);
            _root = *nhd;
            ...
            // std::cout<<"total items:"<<total_items<<" empty items:"<<empty_items<<std::endl;
            return lipp_size;
        }
        typedef struct {
            int block;
