 ...
 private:
  ...
  void ComputeRadixTableStatistics(std::vector<Statistics>& statistics) {
    ...
    for (unsigned index = 1; index <= maxNumRadixBits; ++index) {
      radixAnalyzer[index].shiftBits = GetNumShiftBits((uint64_t)max_key_ - (uint64_t)min_key_, index);
  ...
  void ComputeCHTStatistics(std::vector<Statistics>& statistics) {
    ...
    const unsigned lg = ComputeLog((uint64_t)max_key_ - (uint64_t)min_key_, true);
    ...
    const auto ExtractLCP = [&](unsigned index) -> unsigned {
      return ComputeLcp((uint64_t)spline_points_[index].x - (uint64_t)min_key_, (uint64_t)spline_points_[index - 1].x - (uint64_t)min_key_) - alreadyCommon;// __builtin_clzl((spline_points_[index].x - min_key_) ^ (spline_points_[index - 1].x - min_key_)) - alreadyCommon;
    };
