...
const long long *interval::data = NULL;
...
double interval::init_lr(bool force_init) {
    ...
    if (!lr) {
        ...
        if (lSib) {
            lr->init((long*)data, lSib->start_idx, start_idx, fanout);
        } else {
            lr->init((long*)data, start_idx, start_idx, fanout);
        }
        return lr->cal_loss((long*)data + start_idx, fanout);
    } else {
        return linear_loss;
    }
}
...
double interval::init_lr_w_sampling(bool force_init) {
    ...
    if (!lr) {
        lr = new linearRegressor();
        if (lSib) {
            lr->init_w_sampling((long*)data, lSib->start_idx, start_idx, fanout);
        } else {
            lr->init_w_sampling((long*)data, start_idx, start_idx, fanout);
        }
        return lr->cal_loss_w_sampling((long*)data + start_idx, fanout);
    } else {
        return linear_loss;
    }
}
