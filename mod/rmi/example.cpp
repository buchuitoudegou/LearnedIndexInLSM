...
int main()
{
    ...
    for(int i=0; i<keys.size();i++){
        ...
        if(std::distance(keys.begin(), pos) != i)
            std::cout << "Key number: "<<i <<", value: " << key << " is not found."<<std::endl;
        else
            std::cout << "Key number: "<<i <<", value: " << key << " is found."<<std::endl;
    }

    std::string filename = "rmi.txt";
    system("rm -rf rmi.txt");
    rmi.write_file_e(filename); 
    std::cout<<"Finish writing...:"<<std::endl;

    rmi::RmiLAbs<key_type, layer1_type, layer2_type> rmi2;
    std::cout<<"Begin reading...:"<<std::endl;
    rmi2.read_file_e(filename);

    for(int i=0; i<keys.size();i++){
        key_type key = keys[i];

        // Perform a lookup.
        auto range = rmi2.search(key);
        auto pos = std::lower_bound(keys.begin() + range.lo, keys.begin() + range.hi, key);
        // std::cout << "Key " << key << " is located at position "
        //         << std::distance(keys.begin(), pos) << '.' << std::endl;
        if(std::distance(keys.begin(), pos) != i)
