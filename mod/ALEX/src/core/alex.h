...
//      int max_data_node_slots = (1 << 15) / sizeof(V);
 ...
 public:
  ...
  Alex(int _hybrid_mode, bool is_first, char * index_name, char *data_file) {
      ...
      load_metanode();
      if(!is_first){
        int data_file_size = data_node_sm->get_file_size();
        data_node_sm-> TOTAL_BLOCK = data_file_size/BlockSize;
        // std::cout<<"data_file_size:"<<data_file_size<<" TOTAL_BLOCK:"<<data_node_sm->TOTAL_BLOCK<<std::endl;
        data_node_sm->readall();
        sm->TOTAL_BLOCK = sm->get_file_size()/BlockSize;
        // std::cout<<"sm_file_size:"<<sm->get_file_size()<<" TOTAL_BLOCK:"<<sm->TOTAL_BLOCK<<std::endl;
        sm->readall();
      }
...
//    }
    ...
    if (dnh.pre_leaf_block != -1) {
      ...
      // std::vector<char> tmp = data_node_sm->ALLDATA[dnh.pre_leaf_block];
    ...
    if (dnh.next_leaf_block > 0) {
      ...
      std::cout<<"Position1 reading block: "<<dnh.next_leaf_block<<std::endl;
      std::vector<char> tmp = data_node_sm->ALLDATA[dnh.next_leaf_block];
      // std::cout<<"tmp.size is:"<<tmp.size()<<std::endl;
      std::copy(tmp.begin(), tmp.end(), data);
...
//      }
  ...
  P get_key_v(DataHeaderOnDisk *data_node, int pred_pos, int _block_, int _offset_, int *last_block, char *data, int *r_block_count) {
      long key_offset = _block_ * BlockSize + _offset_ + DataHeaderOnDiskSize
                      + data_node->empty_byte_count_1 + data_node->bitmap_size * sizeof(uint64_t)
                      + data_node->empty_byte_count_2 + pred_pos * ItemOnDiskSize;
      int key_block = key_offset / BlockSize;
      ...
      P pred_key_v = ((ItemOnDisk *)(data + key_offset_in_block))->value;
      return pred_key_v;
  }
...
//      int last_block = _block_;
      if (_key_greater(get_key(data_node, pred_pos, _block_, _offset_, last_block, data, r_block_count), key)) {
          ...
          r = pred_pos - bound / 2;
          // std::cout<<"greater l:"<<l<<" r:"<<r<<std::endl;
      } else {
          ...
          // std::cout<<"smaller l:"<<l<<" r:"<<r<<std::endl;
      }
      // binary search
      // std::cout<<"binary l:"<<l<<" r:"<<r<<std::endl;
      while (l < r) {
          int mid = l + (r -l) / 2;
      ...
      T key_get = get_key(data_node, l, _block_, _offset_, last_block, data, r_block_count);
      // std::cout<<"Keyget: "<<key_get<<std::endl;
      P value_get = get_key_v(data_node, l, _block_, _offset_, last_block, data, r_block_count);
      // std::cout<<"Valueget: "<<value_get<<std::endl;

      // if(l<0){
      //   P V_neg = get_key_v(data_node, l, _block_, _offset_, last_block, data, r_block_count);
      ...
      //   return nullptr;
      // }
      // std::cout<<"getvalue paras: " <<_block_<<" " << _offset_<<std::endl;

      P _v;
      // std::cout<<"later l:"<<l<<std::endl;

      _v = get_value(data_node, l, _block_, _offset_, data);
      // std::cout<<" V pos: "<<_v<<std::endl;
      


      
      // std::cout<<"_v in get_value: "<< _v<<std::endl; 
  ...
  P *get_payload_on_disk(T key, int block, int offset, char *data, int *r_block_count, int *POS = nullptr) {
    ...
    // std::cout<<"Min key: "<< _data_node.first_key<<" max key: "<<_data_node.last_key<<std::endl;
    // std::cout<<"pred_pos_d: "<<pred_pos_d<< " capa: "<< _data_node.data_capacity <<std::endl;
...
//      if (!same) {
          ...
          std::vector<char> tmp = data_node_sm->ALLDATA[block];
          std::copy(tmp.begin(), tmp.end(), data);
          *r_block_count += 1;
//      }
      *r_level += 1;
      ...
      if (std::abs(pred_pos_d - pred_pos_i_rounded) <= tolerance) {
          if (pred_pos_i_rounded <= pred_pos_d) {
              if (dh->pre_leaf_block != -1) {
                  ...
                  std::vector<char> tmp = data_node_sm->ALLDATA[dh->pre_leaf_block];
                  std::copy(tmp.begin(), tmp.end(), pre_node_data);
                  *r_block_count += 1;
              ...
              if (dh->next_leaf_block != -1) {
                  ...
                  std::vector<char> tmp = data_node_sm->ALLDATA[dh->next_leaf_block];
                  std::copy(tmp.begin(), tmp.end(), next_node_data);
  ...
  bool get_leaf_disk_all(T key, P *v, int *r_block_count, int *r_level, int *r_inner_block) {
    ...
    std::vector<char> tmpsm = sm->ALLDATA[block];
    std::copy(tmpsm.begin(), tmpsm.end(), data);
    // std::cout<<"modeldata:"<<data<<std::endl;
    *r_block_count += 1;
    *r_inner_block += 1;
    *r_level += 1;
    ...
    while (true) {
      ...
      if (block_in_inode != block) {
        ...
        std::vector<char> tmpsm = sm->ALLDATA[block_in_inode];
        std::copy(tmpsm.begin(), tmpsm.end(), data);
        *r_block_count += 1;
        *r_inner_block += 1;
...
// node's parent.
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    ...
    // std::cout<<"In index bulkload() numkeys: "<<num_keys<<std::endl;
 ...
 private:
  ...
  bool bulk_load_node(const V values[], int num_keys, AlexNode<T, P>*& node,
    ...
    if (num_keys <= derived_params_.max_data_node_slots *
      ...
      // std::cout<<"In index bulkloadnode() numkeys: "<<num_keys<<std::endl;
...
//    data_node_sm->read_arbitrary(&_bt, _start_offset_, sizeof(uint64_t));
  ...
  std::pair<int, int> insert_into_leaf_node(T key, P value, char *data, DataHeaderOnDisk *leaf_node, PhyscialAddr *addr,
    ...
    if (leaf_node->num_keys >=leaf_node->expansion_threshold_) {
      ...
      std::cout<<"Position2 reading block: "<<addr->block<<std::endl;
      std::vector<char> tmp = data_node_sm->ALLDATA[addr->block];
      // std::cout<<"tmp.size is:"<<tmp.size()<<std::endl;
      std::copy(tmp.begin(), tmp.end(), data2);
      int c = 0;
      int pos;
      int upper_bound_pos;
