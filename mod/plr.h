...
private:
    ...
    Segment process__(struct point pt, bool file, int key_tag);

public:
    GreedyPLR(double gamma);
    Segment process(const struct point& pt, bool file, int key_tag);
    Segment finish();
};
...
public:
    PLR(double gamma);
    std::vector<Segment>& train(std::vector<std::pair<std::string,int>>& keys, bool file);
