...
int main() {
...

    {
        ...
        pgm::MappedPGMIndex<int, 32> pgm2(tmp_filename);
        std::cout << "File re-opened" << std::endl
                  << "count(15) = " << pgm2.count(15) << std::endl
                  << "contains(1) = " << (pgm2.contains(1) ? "true" : "false") << std::endl
                  << "lower_bound(42) = " << *pgm2.lower_bound(42) << std::endl
                  << "upper_bound(70) = " << *pgm2.upper_bound(70) << std::endl
                  << "Range search [50, 60] = ";
        print_iterator<int>(pgm2.lower_bound(50), pgm2.upper_bound(60));
    }
