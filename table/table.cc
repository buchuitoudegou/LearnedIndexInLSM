...
void Table::FillData(const ReadOptions& options, adgMod::LearnedIndexData* data) {
  ...
  // printf("index block num_restarts: %lu\n", (unsigned long)index_iter->num_restarts_);
  for (uint32_t i = 0; i < index_iter->num_restarts_; ++i) {
    ...
    for (block_iter->SeekToRestartPoint(0); block_iter->ParseNextKey(); ++num_entries_this_block) {
      ...
      if(adgMod::nofence == 1){
        if(first_key_flag){
          ...
          data->string_keys.emplace_back(std::make_pair(std::string(parsed_key.user_key.data(), parsed_key.user_key.size()), 0));
          // std::cout<<"string_keys number "<<num_entries_this_block<<" out of "<<num_points+num_entries_this_block<<" emplace_back as 0:"<<std::string(parsed_key.user_key.data(), parsed_key.user_key.size())<<std::endl;
        }
    ...
    if(adgMod::nofence == 1 || adgMod::nofence == 2){
      data->string_keys.back().second = 2;
      // std::cout<<"string_keys number "<<num_entries_this_block-1<<" out of "<<num_points+num_entries_this_block-1<<" emplace_back as 2:"<<data->string_keys.back().first<<std::endl;
    }
    // std::cout<<"block entry num "<<num_entries_this_block<<std::endl;
    num_points += num_entries_this_block;

    if(num_entries_this_block > adgMod::block_num_entries){
